...
class AzureDriver(driver.ComputeDriver):
    ...
    def init_host(self, host):
        ...
        self.flavor_info = azutils.get_vm_sizes(self.compute_client,
                                                drv_conf.zone)
        LOG.info("%s driver init with %s project, %s zone" %
                 (self.name, drv_conf.tenant_id, drv_conf.zone))
    ...
    def _get_omni_id_from_instance(self, instance):
        ...
        elif instance.uuid in self._uuid_to_instance:
            return self._uuid_to_instance[instance.uuid]['name']
    ...
    def list_instances(self):
        ...
        """
        compute, project, zone = self.azure_svc, self.gce_project, self.gce_zone
        all_instances = azureutils.list_instances(compute, project, zone)
        self._uuid_to_azure_instance.clear()
        instance_names = []
        for instance in all_instances:
            if instance['status'] in [
                ...
                continue
            openstack_id = azureutils.get_instances_metadata_key(
                compute, project, zone, instance, 'openstack_id')
            if openstack_id is None:
                openstack_id = self._get_uuid_from_azure_id(instance['name'])
            self._uuid_to_azure_instance[openstack_id] = instance
    ...
    def get_host_stats(self, refresh=False):
        ...
        for nodename in _DRV_NODES:
            ...
            host_status['hypervisor_type'] = 'Azure'
    ...
    def list_instance_uuids(self, node=None, template_uuids=None, force=False):
        ...
        for instance in all_instances:
            ...
            self._uuid_to_azure_instance[openstack_id] = instance
            self._uuid_to_azure_instance[openstack_id] = instance
        return self._uuid_to_azure_instance.keys()
        return self._uuid_to_azure_instance.keys()
        """
