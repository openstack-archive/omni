...
class AzureDriver(driver.ComputeDriver):
    ...
    def _get_network_profile(self, network_info):
        ...
        # TODO(ssudake21): Support for multiple network interfaces
        net_info = network_info[0]
        for net_info in network_info:
            azure_network_name = 'net-' + net_info['network']['id']
            azure_subnet_name = 'subnet-' + net_info['details']['subnet_id']
            ip_address = net_info['details']['ip_address']
            nic_name = 'nic-' + net_info['id']
            ipc_name = 'ipc-' + net_info['id']
            azure_subnet = azutils.get_subnet(
                self.network_client, drv_conf.resource_group,
                azure_network_name, azure_subnet_name)
            LOG.info("Azure subnet %s" % (azure_subnet, ))
            body = {
                'location':
                drv_conf.region,
                'ip_configurations': [{
            ...
            nic = azutils.create_nic(self.network_client,
                                     drv_conf.resource_group, nic_name, body)
    ...
    def _get_storage_profile(self, instance):
        ...
        attrs_list = img_link.strip('/').split('/')
        attrs_dict = {
        ...
        return {
            'image_reference': {
                'publisher': attrs_dict['publishers'],
                'offer': attrs_dict['offers'],
                'sku': attrs_dict['skus'],
                'version': attrs_dict['versions']
            }
        }

    def _get_os_profile(self, instance):
        # TODO(ssudake21): Use ssh keys instead
        return {
            'computer_name': self._azure_instance_name(instance),
            'admin_username': 'userlogin',
            'admin_password': 'Pa$$w0rd91'
        }

    def _prepare_vm_params(self, instance, network_info):
        os_profile = self._get_os_profile(instance)
    ...
    def spawn(self,
        ...
        azutils.create_or_update_instance(self.compute_client, drv_conf.resource_group, name, tags)
        instance.metadata.update({OMNI_NAME: name})
        self._uuid_to_omni_instance[instance.uuid] = azutils.get_instance(
            self.compute_client, drv_conf.resource_group, name)
    ...
    def destroy(self,
        ...
        try:
            ...
            return
        try:
            azutils.delete_instance(self.compute_client,
                                    drv_conf.resource_group, azure_name)
        except azutils.CloudError:
            LOG.error(
    ...
    def get_info(self, instance):
        ...
        azure_name = self._get_omni_name_from_instance(instance)
        az_instance = azutils.get_instance(self.compute_client, drv_conf.resource_group, azure_name)
        LOG.info("get instance %s" % az_instance)
        statuses = az_instance.instance_view.statuses
        state = constants.power_state.NOSTATE
        for i in statuses:
            if hasattr(i, 'code') and 'PowerState' in i.code:
                state = constants.OMNI_STATE_MAP[i.code]
        # TODO(ssudake21): Report correct info
        return hardware.InstanceInfo(state=state, id=instance.id)
