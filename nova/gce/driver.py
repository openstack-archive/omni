...
class GCEDriver(driver.ComputeDriver):
    ...
    def snapshot(self, context, instance, image_id, update_task_state):
        ...
        try:
            ...
            LOG.debug(
                _LI("nova snapshot: 1. Found boot disk %s for instance %s") %
            ...
            LOG.debug(
                _LI("nova snapshot: 2. Temporarily stopped instance %s") %
                instance.uuid)

            snapshot_name = 'novasnap-' + disk_name
            operation = gceutils.snapshot_disk(
                compute, project, zone, boot_disk['name'], snapshot_name)
            gceutils.wait_for_operation(compute, project, operation)
            temp_disk_snapshot = True
            LOG.debug(
                _LI("nova snapshot: 3. Created boot disk snapshot %s") %
                snapshot_name)
            ...
            instance_stopped = False
            LOG.debug(
                _LI("nova snapshot: 4. Restart instance after disk snapshot %s"
                    ) % instance.uuid)
            ...
            LOG.debug(
                _LI("nova snapshot: 5. Created disk %s from snapshot %s") %
            ...
            image_created = True
            LOG.debug(
                _LI("nova snapshot: 6. Created image %s from disk %s") %
            ...
            LOG.debug(_LI("nova snapshot: 7. Added image to glance %s") % name)
            ...
            temp_disk_from_snapshot = False
            LOG.debug(
                _LI("nova snapshot: 8. Delete temporary disk %s") %
                snapshot_disk_name)

            gceutils.wait_for_operation(compute, project, snap_operation)
            temp_disk_snapshot = False
            LOG.debug(
                _LI("nova snapshot: 9. Delete temporary disk snapshot %s") %
                snapshot_name)
            ...
            if instance_stopped:
                ...
                LOG.debug(
                    ...
                    instance.uuid)
            if image_created:
                LOG.debug(_LI("Delete image %s from GCE") % name)
            ...
            if temp_disk_from_snapshot:
                disk_operation = gceutils.delete_disk(compute, project, zone,
                                                      snapshot_disk_name)
                gceutils.wait_for_operation(compute, project, disk_operation)
                LOG.debug(_LI("Delete temporary disk %s") % snapshot_disk_name)
            if temp_disk_snapshot:
                snap_operation = gceutils.delete_snapshot(
                    compute, project, snapshot_name)
                gceutils.wait_for_operation(compute, project, snap_operation)
                LOG.debug(
                    _LI("Delete temporary disk snapshot %s") % snapshot_name)
            raise e
