...
class AzureDriver(BaseVD):
    ...
    def do_setup(self, context):
        self.connection = azureutils.get_azure_client(
    ...
    def create_volume(self, volume):
        ...
        azureutils.create_disk(self.connection, self.resource_group,
                               self.region, volume_name, volume['size'])
        # Validating that Volume is created in Azure
        _ = azureutils.get_disk(self.connection, self.resource_group,
                                volume_name)
    ...
    def create_volume_from_snapshot(self, volume, snapshot):
        ...
        azureutils.create_disk_from_snapshot(
            self.connection, self.resource_group, self.region, volume_name,
            snapshot_name)
        _ = azureutils.get_disk(self.connection, self.resource_group,
                                volume_name)

    def create_cloned_volume(self, volume, src_vref):
        azureutils.create_disk_from_disk(self.connection, self.resource_group,
    ...
    def delete_volume(self, volume):
        volume_name = self._azure_volume_name(volume)
        azureutils.delete_disk(self.connection, self.resource_group,
                               volume_name)
        try:
            disk_details = azureutils.get_disk(
                self.connection, self.resource_group, volume_name)
            if disk_details:
                raise Exception("Delete volume failed for '{0}'".format(
                    volume_name))
        except CloudError:
    ...
    def create_snapshot(self, snapshot):
        ...
        snapshot_name = self._azure_snapshot_name(snapshot)
        azureutils.snapshot_disk(self.connection, self.resource_group,
                                 self.region, volume_name, snapshot_name)
        _ = azureutils.get_snapshot(self.connection, self.resource_group,
                                    snapshot_name)

    def delete_snapshot(self, snapshot):
        snapshot_name = self._azure_snapshot_name(snapshot)
        azureutils.delete_snapshot(
            self.connection, self.resource_group, snapshot_name)
        try:
            snapshot_details = azureutils.get_snapshot(
                self.connection, self.resource_group, snapshot_name)
            if snapshot_details:
                raise Exception("Delete snapshot failed '{0}'".format(
                    snapshot_name))
        except CloudError:
            pass
