...
def create_disk(compute, resource_group, region, disk_name, size):
    ...
    try:
        async_action = compute.disks.create_or_update(
...
def delete_disk(compute, resource_group, disk_name):
    ...
    try:
        ...
        _ = get_disk(compute, resource_group, disk_name)
        async_action = compute.disks.delete(resource_group, disk_name)
        LOG.debug("Response: {0}".format(async_action.result()))
...
def snapshot_disk(compute, resource_group, region, disk_name, snapshot_name):
    ...
    """
    try:
        disk_info = get_disk(compute, resource_group, disk_name)
        data = {
            'location': region,
            'creation_data': {
        ...
        async_action = compute.snapshots.create_or_update(
...
def delete_snapshot(compute, resource_group, snapshot_name):
    ...
    try:
        ...
        _ = get_snapshot(compute, resource_group, snapshot_name)
        async_action = compute.snapshots.delete(resource_group, snapshot_name)
        LOG.debug("Response: {0}".format(async_action.result()))
...
def create_disk_from_snapshot(compute, resource_group, region, disk_name,
                              snapshot_name):
