...
class GceRouterPlugin(
    ...
    def create_floatingip(self, context, floatingip):
        ...
        try:
            ...
            LOG.error("Error in Creation/Allocating floating IP")
        ...
        try:
            ...
            LOG.error("Error in adding floating IP to openstack")
            if public_ip_allocated:
                self._cleanup_floatingip(compute, project, region,
                                         public_ip_allocated)
            raise e
    ...
    def _associate_floatingip_to_port(self, context, floating_ip_address,
        ...
        if fixed_ip_address:
            LOG.info('Found fixed ip %s for port %s' % (fixed_ip_address,
                                                        port_id))
            gceutils.assign_floatingip(compute, project, zone,
                                       fixed_ip_address, floating_ip_address)
        else:
            LOG.warning('Could not find any fixed ip for port %s' % port_id)
            raise Exception('No server found for IP assignment')

    def update_floatingip(self, context, id, floatingip):
        LOG.debug('update_floatingip id %s floatingip %s' % (id, floatingip, ))
        floatingip_dict = floatingip['floatingip']
    ...
    def delete_floatingip(self, context, id):
        LOG.debug('delete_floatingip %s' % id)
