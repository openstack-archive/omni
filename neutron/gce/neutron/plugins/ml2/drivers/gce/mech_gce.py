...
class GceMechanismDriver(api.MechanismDriver):
    ...
    def _convert_secgrp_rule_to_gce(self, rule, network_link):
        if rule['ethertype'] != 'IPv4' or rule['direction'] != 'ingress':
            # Not supported yet by GCE
            LOG.warning(
                _LI("Unsupported ethertype %s or direction %s in GCE security rule conversion."
                    ) % (rule['ethertype'], rule['direction']))
            return {}
        ...
        directions = {'ingress': 'INGRESS', 'egress': 'EGRESS'}
        ...
        if rule['direction'] in directions:
            ...
            LOG.warning(
                _LI("Unknown direction %s in GCE security rule conversion.") %
                rule['direction'])
            return {}
        ...
        if protocol is None:
            ...
            LOG.warning(
                _LI("Unknown protocol %s in GCE security rule conversion.") %
                protocol)
            return {}
    ...
    def _create_secgrp_rule(self, context, rule, network_link):
        ...
        if gce_rule:
            ...
            operation = gceutils.create_firewall_rule(compute, project,
                                                      gce_rule)
            gceutils.wait_for_operation(compute, project, operation)
    ...
    def _update_secgrp_rule(self, context, rule_id):
        ...
        if gce_rule:
            ...
            LOG.info(_LI("Delete GCE firewall rule %s") % name)
    ...
    def _delete_secgrp_rule(self, context, rule_id):
        ...
        try:
            LOG.warn(
                _LI("Delete exiting GCE firewall rule %s,"
