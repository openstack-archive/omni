...
class GceMechanismDriver(api.MechanismDriver):
    ...
    def _convert_secgrp_rule_to_gce(self, rule, network_link):
        ...
        if protocol is None:
            ...
            LOG.warning(
                _LI("Unknown protocol in GCE security rule conversion %s.") %
                protocol)
            return {}
    ...
    def _create_secgrp_rules_if_needed(self, context, secgrp_ids):
        ...
        if secgrp_rules:
            ...
            for secgrp_rule in secgrp_rules:
                self._create_secgrp_rule(context, secgrp_rule, network_link)

    def _delete_secgrp(self, context, secgrp_id):
        # TODO: Delete secgrp rules corresponding to secgrp id
        pass
    ...
    def secgroup_callback(self, resource, event, trigger, **kwargs):
        # Remove later
        LOG.info(
            "SSS {0} {1} {2} {3}".format(resource, event, trigger, kwargs))
