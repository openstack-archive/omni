...
class GceResourceNotFound(Exception):
    pass
...
def get_floatingip(compute, project, region, ip):
    ...
    raise GceResourceNotFound('Floating IP %s not found' % ip)
...
def assign_floatingip(compute, project, zone, fixedip, floatingip):
    ...
    if not instance_name:
        raise GceResourceNotFound(
            'Instance with fixed ip %s not found' % fixedip)
