...
class AzureRouterPlugin(
    ...
    def _add_floatingip_to_port(self, port_id, public_ip):
        ...
        nic = azutils.get_nic(net_svc, resource_group, nic_name)
        nic.ip_configurations[0].public_ip_address = public_ip
        azutils.update_nic(net_svc, resource_group, nic_name, nic)
    ...
    def create_floatingip(self, context, floatingip):
        ...
        try:
            ...
            public_ip = azutils.allocate_floatingip(net_svc, resource_group,
                                                    self.region)
            ...
            if public_ip:
                azutils.delete_floatingip(net_svc, resource_group,
                                          public_ip.name)
            raise e
        return res
    ...
    def update_floatingip(self, context, id, floatingip):
        ...
        port_id = floatingip_dict['port_id']
        if port_id:
            public_ip = azutils.get_floatingip(net_svc, resource_group,
                                               ip_address)
            self._add_floatingip_to_port(port_id, public_ip)
            status = n_const.FLOATINGIP_STATUS_ACTIVE
        else:
    ...
    def delete_floatingip(self, context, id):
        ...
        public_ip = azutils.get_floatingip(net_svc, resource_group, ip_address)
        port_id = floating_ip['port_id']
        if port_id:
            self._remove_floatingip_from_port(port_id)
        azutils.delete_floatingip(net_svc, resource_group, public_ip.name)
